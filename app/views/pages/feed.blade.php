...
<channel>
    ...
    <item>
        ...
        <link>
        {{ Config::get('app.url') }}/snippet/{{ $snippet->id }}</link>
        ...
        <content:encoded><![CDATA[<p>{{ Text::renderForRss($snippet->description) }}</p>

            <p>
            <pre>{{ htmlentities($snippet->body) }}</pre>
            </p>]]>
        </content:encoded>
    </item>
    @endforeach
